...
struct PerformanceCurveFitter {
    ...
    static func fitPowerLaw(from points: [PerformancePoint]) -> PowerLawModel? {
        let valid = points.filter { $0.output.unit is UnitSpeed || $0.output.unit is UnitPower }
        guard valid.count >= 2 else { return nil }
        
        let logT = valid.map { log($0.duration) }
        let logV = valid.map { log($0.output.value) }
    ...
    static func fit3PModel(from points: [PerformancePoint]) -> CriticalPowerModel? {
        let valid = points.filter { $0.output.unit == UnitPower.watts }
            ...
            for wp in wpRange {
                for k in kRange {
                    let error = valid.reduce(0.0) { sum, pt in
                        ...
                        return sum + pow(predicted - pt.output.value, 2)
                    }
                    if error < minError {
                        minError = error
                        bestParams = (cp, wp, k)
    ...
    static func fitSwimModel(from points: [PerformancePoint]) -> SwimDecayModel? {
        let valid = points.filter { $0.output.unit == UnitSpeed.metersPerSecond }
        guard valid.count >= 3 else { return nil }

        let css = valid.max(by: { $0.duration < $1.duration })?.output.value ?? 1.3
        ...
        for delta in stride(from: 0.01, through: 0.5, by: 0.01) {
            for k in stride(from: 0.001, through: 0.01, by: 0.001) {
                let error = valid.reduce(0.0) { sum, pt in
                    ...
                    return sum + pow(v - pt.output.value, 2)
                }
                if error < minError {
                    minError = error
                    best = (delta, k)
